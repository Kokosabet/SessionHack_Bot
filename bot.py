...
client = TelegramClient('@T_G_L', api_id, api_hash).start(bot_token=token)
...
**NOTICE JOIN @ReallyKoko , @T_G_L FEDERATION**
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      await event.reply("      Thanks For using @T_G_L Bot")
    elif r == "F":
      await x.send_message("   ")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      if i:
        await event.reply("         `LEGENDXISBEST` \n\nThanks For using @T_G_L Bot")
      else:
        await event.reply("    .")
    elif r == "I":
      await x.send_message("   ")
